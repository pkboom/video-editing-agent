...
def main():
    ...
    with col2:
        ...
        if st.button(
            ...
            help="Process the selected video file" if not run_disabled else "Please select a file first"
        ):
            if uploaded_file is not None:
...
async def process_video(uploaded_file, user_script: str):
    ...
    try:
        ...
        if Path(transcription_file_path).exists():
            ...
            st.write(f"Transcription file size: {Path(transcription_file_path).stat().st_size / 1024:.2f} KB")
        else:
        ...
        processed_res = await process_transcription_with_llm(transcription_file_path, user_script)
